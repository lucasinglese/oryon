...
fn main() {
    let prices: Vec<Option<f64>> = vec![
        ...
        Some(110.0), Some(111.5), Some(113.0), Some(112.0), Some(114.5),
    ];
    ...
    println!("{:<5} {:<8} {:<10} {:<12} {}", "bar", "close", "log_ret", "lr_sma5", "ema5");
    for (i, (row, &price)) in matrix.iter().zip(prices.iter()).enumerate() {
        println!("{:<5} {:<8.2} {:<10} {:<12} {}", i, price.unwrap(), fmt(row[0]), fmt(row[1]), fmt(row[2]));
    }
    ...
    for (i, &price) in prices.iter().enumerate() {
        ...
        println!("bar {i}: close={:.2}  lr={}  lr_sma5={}  ema5={}",
            price.unwrap(), fmt(out[0]), fmt(out[1]), fmt(out[2]));
    }
}
