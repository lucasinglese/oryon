...
fn main() {
    ...
    let pv    = ParkinsonVolatility::new(vec!["high".into(), "low".into()], 5, vec!["pv5".into()]).unwrap();
    ...
    println!("{:<5} {:<8} {:<8} {:<12} {:<12} {:<12} {}",
        "bar", "high", "low", "pv5", "hl_slope", "hl_r2", "");
    for i in 0..highs.len() {
        let out = pipeline.update(&[highs[i], lows[i]]);
        println!("{:<5} {:<8.1} {:<8.1} {:<12} {:<12} {}",
            i, highs[i].unwrap(), lows[i].unwrap(),
            fmt(out[0]), fmt(out[2]), fmt(out[3]),
        );
    }
}
