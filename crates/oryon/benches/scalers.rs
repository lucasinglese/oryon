...
fn bench_rolling_zscore(c: &mut Criterion) {
    ...
    let mut rz_w20 = RollingZScore::new(vec!["x".into()], 20, vec!["x_z".into()]).unwrap();
    group.bench_function("w20", |b| {
        b.iter(|| rz_w20.update(black_box(&[Some(100.0)])))
    });

    let mut rz_w200 = RollingZScore::new(vec!["x".into()], 200, vec!["x_z".into()]).unwrap();
...
fn bench_fixed_zscore(c: &mut Criterion) {
    ...
    let mut fz = FixedZScore::new(vec!["x".into()], vec!["x_z".into()], params).unwrap();
