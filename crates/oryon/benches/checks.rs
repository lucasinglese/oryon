...
fn bench_is_none(c: &mut Criterion) {
    c.bench_function("is_none", |b| {
        b.iter(|| is_none(black_box(Some(100.0))))
    });
}

fn bench_is_inf(c: &mut Criterion) {
    c.bench_function("is_inf", |b| {
        b.iter(|| is_inf(black_box(100.0)))
    });
}

fn bench_is_valid(c: &mut Criterion) {
    c.bench_function("is_valid", |b| {
        b.iter(|| is_valid(black_box(Some(100.0))))
    });
}
