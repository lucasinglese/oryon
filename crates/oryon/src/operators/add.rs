...
mod tests {
    ...
    streaming_transform_contract_tests!(
        Add::new(vec!["a".into(), "b".into()], vec!["a_plus_b".into()]).unwrap(),
        vec!["a".to_string(), "b".to_string()],
        vec!["a_plus_b".to_string()],
        0,
        &[Some(1.0), Some(2.0)],
    );
    ...
    fn test_update() {
        let mut op = add();
        assert_eq!(op.update(&[Some(3.0), Some(2.0)]), out(Some(5.0)));
        assert_eq!(op.update(&[Some(-1.0), Some(4.0)]), out(Some(3.0)));
        assert_eq!(op.update(&[Some(0.0), Some(0.0)]), out(Some(0.0)));
    }
    ...
    fn test_fresh_instances_are_independent() {
        ...
        assert_eq!(fresh.update(&[Some(3.0), Some(2.0)]), out(Some(5.0)));
        assert_eq!(op.update(&[Some(10.0), Some(5.0)]), out(Some(15.0)));
    }

    #[test]
    fn test_error_raises_when_inputs_wrong_length() {
        assert!(matches!(
            Add::new(vec!["a".into()], vec!["out".into()]).unwrap_err(),
            crate::error::OryonError::InvalidInput { ref msg } if msg.contains("2 columns")
        ));
        assert!(matches!(
            Add::new(vec!["a".into(), "b".into(), "c".into()], vec!["out".into()]).unwrap_err(),
            crate::error::OryonError::InvalidInput { ref msg } if msg.contains("2 columns")
        ));
    }
    ...
    fn test_error_raises_when_outputs_empty() {
        assert!(matches!(
            Add::new(vec!["a".into(), "b".into()], vec![]).unwrap_err(),
