...
use features::{
    Adf, AutoCorrelation, Correlation, Ema, Kama, Kurtosis, LinearSlope, LogReturn, Mma,
    ParkinsonVolatility, RogersSatchellVolatility, ShannonEntropy, SimpleReturn, Skewness, Sma,
};
...
pub(crate) fn extract_feature(obj: &Bound<'_, PyAny>) -> PyResult<Box<dyn StreamingTransform>> {
    ...
    if let Ok(f) = obj.extract::<PyRef<AutoCorrelation>>() {
        return Ok(f.inner.fresh());
    }
    // operators
